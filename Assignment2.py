...
# contains the label.
data = np.loadtxt(os.path.join(r'C:\Users\zeina\Desktop\Assignment2\Data', 'ex2data1.txt'), delimiter=',')
X, y = data[:, 0:2], data[:, 2]
...
def costFunction(theta, X, y):
    ...
    a=np.dot(X,theta)
    h=sigmoid(a)
    s = np.sum(-y*np.log(h) - (1-y)*np.log(1-h))
    J = (1 / (m)) * (s)
...
data = np.loadtxt(os.path.join(r'C:\Users\zeina\Desktop\Assignment2\Data', 'ex2data2.txt'), delimiter=',')
X = data[:, :2]
y = data[:, 2]
